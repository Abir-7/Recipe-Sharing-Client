...
const RatingOperation = ({
  ...
  const editDeletComent = (data: {
    ...
    if ((data.oldComment && data.newComment) || data.newComment == "true") {
      handleRating({ ...data });
    } else {
    ...
    <div>
      {isOpen && (
        <div className="  min-h-40 max-h-96  rounded-lg p-2 shadow-inner ">
          <div className="max-h-80 min-h-20 rounded-lg px-4 py-2 bg-gray-900 overflow-y-auto">
            {" "}
            {comments?.map((cmt, i) => (
              <p key={i} className="grid ">
                {cmt.comment.length > 0 ? (
                  <>
                    <span>
                      {cmt.comment
                        .filter((c) => c.trim() !== "")
